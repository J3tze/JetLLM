  ...
  return (
    <div className="flex gap-2 flex-1 sm:flex-initial min-w-0">
      <Select value={provider} onValueChange={onProviderChange}>
        <SelectTrigger className="w-[120px] sm:w-[140px] h-8 text-xs bg-card shrink-0">
      ...
      {loading ? (
        <div className="min-w-0 flex-1 sm:w-[200px] h-8 flex items-center px-3 text-xs text-muted-foreground bg-card rounded-md border border-input">
          Loading models...
        </div>
      ) : showSearchable ? (
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button
              ...
              role="combobox"
              aria-expanded={open}
              className="min-w-0 flex-1 sm:w-[200px] h-8 justify-between text-xs bg-card font-normal"
            >
        ...
        <Select value={model} onValueChange={onModelChange}>
          <SelectTrigger className="min-w-0 flex-1 sm:w-[200px] h-8 text-xs bg-card">
        ...
        <Input
          ...
          className="min-w-0 flex-1 sm:w-[200px] h-8 text-xs bg-card"
        />
      )}
    </div>
  )
